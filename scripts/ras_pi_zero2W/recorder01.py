            ...
            if button.get_State()[0]:
                if not self.rec :
                    ...
                    result = cv.VideoWriter(
                        ...
                        cv.VideoWriter_fourcc(*'XVID'), fps, (640,480))
...
if __name__ == "__main__":
    ...
    try:
        print('We are On ')
        button=Button(pin=4)
        molette=RotaryEncoder(clk_pin=27, dt_pin=22)
        
        err_led=LED(pins=[26])
        on_led=LED(pins=[25])
        rec_led=LED(pins=[24],blink_interval=10)
        upld_led=LED(pins=[23],blink_interval=10)
